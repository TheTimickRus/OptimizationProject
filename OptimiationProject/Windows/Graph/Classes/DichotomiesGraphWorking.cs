...
{
    public static class DichotomiesGraphWorking
    { 
        public static void MainWorking(GraphModel graphModel)
        {
            ...
            {
                var alg = new DichotomiesAlg(graphModel.FuncStr, paramsMain[0],
                                                                 paramsMain[1],
                                                                 paramsMain[2],
                                                                 paramsMain[3]);
                ...
                {
                    case 0:
                        alg.LowerLimit = current; alg.MainWorking();

                        xPoint.Add(alg.LowerLimit);
                        break;

                    case 1:
                        alg.UpperLimit = current; alg.MainWorking();

                        xPoint.Add(alg.UpperLimit);
                        break;

                    case 2:
                        alg.Delta = current; alg.MainWorking();

                        xPoint.Add(alg.Delta);
                        break;

                    case 3:
                        alg.Eps = current; alg.MainWorking();
